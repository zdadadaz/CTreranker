...
from utils.training_ance import trainer
...
from src.inference import run_inference_iteration, run_neg_inference
...
def main():
    ...
    if output is None:
        output = os.path.join("output_judge_iteration", "{}_{}_{}".format(eval_year, modelname,
                                                          "pretrained_" + pretrained if pretrained else "random"))
    pathlib.Path(output).mkdir(parents=True, exist_ok=True)
    pathlib.Path(os.path.join(output, 'runs')).mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(epochs):
        if task.split('_')[1] == 'bert':
            if epoch == 0:
                ...
                path_to_top_results = f'{output}/runs/{eval_year}_{epoch-1}_{pretrained}_train_1500.res'
                path_to_top_neg_results = f'{output}/runs/{eval_year}_{epoch-1}_{pretrained}_train_neg.res'
        else:
        ...
        for phase in dataidx:
            ...
            # else:
            #     tmp = [str(i) for i in range(80,90)]
        ...
        trainBERT = trainer(model, pretrained, output, batch_size, epoch+1, dataloaders, device, isFinetune, lr)
        trainBERT.train()

        run_inference_iteration(model, tokenizer, eval_year, 'bm25', output, epoch)
        run_neg_inference(model, tokenizer, eval_year, 'bm25', output, epoch)
