            ...
            for epoch in range(epoch_resume, self.num_epochs):
                ...
                for phase in ['train', 'val']:
                    ...
                    for i in range(torch.cuda.device_count()):
                        ...
                        torch.cuda.reset_peak_memory_stats(i)
                    loss, yhat, y, qdoc = self.run_epoch(self.model, self.dataloaders[phase], phase, self.optim,
                                                      self.device)
                    qrel, run = self._gen_pytrec_run(yhat, y, qdoc)
                    eval = self.evaluate(qrel, run)
                    f.write("{},{},{},{},{},{},{},{}\n".format(epoch,
                                                               phase,
                                                               loss,
                                                               str(eval),
    ...
    def _gen_trec_ouput(self, yhat, qdoc, k):
        ...
        for i in range(0, len(yhat), k):
            ...
            for j in idxlist:
                ...
                res[qid]['score'].append(str(score))
                cnt += 1
        return res

    def _gen_pytrec_run(self, yhat, y, qdoc, k=50):
        res = {}
        qrel = {}
        for i in range(len(yhat)):
            qid, docid = qdoc[i].split('NCT')
            docid = 'NCT' + docid
            if qid not in res:
                res[qid] = {}
            if qid not in qrel:
            ...
            score = float(yhat[i])
            res[qid][docid] = score
            qrel[qid][docid] = int(y[i])
        return (qrel, res)

    def evaluate(self, qrel, run):
        evals = {'map', 'P_10', 'P_15', 'ndcg'}
        evaluator = pytrec_eval.RelevanceEvaluator(qrel, evals)
